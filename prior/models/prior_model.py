...
class persionality_crop_model_multi_scale_shared(nn.Module):
    ...
    def __init__(self, alignsize=8, reddim=32, loadweight=True, model=None, downsample=4, num_classes=1345):
        ...
        # 添加一个新的全连接层
        # self.new_fc = nn.Linear(256, 1)  # 256 = 128 (gaic) + 128 (cpc)

        # # 可能要考虑不同的初始化参数
        # nn.init.xavier_uniform_(self.new_fc.weight)
    ...
    def forward(self, data_gaic, data_cpc):
        ...
        # print(prediction_1.squeeze().shape)
        # print(prediction_2.squeeze().shape)
        # print(prediction_3.squeeze().shape)
