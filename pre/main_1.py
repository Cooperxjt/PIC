...
from models.picModel import build_crop_model
...
def train():
    ...
    for epoch in range(0, 80):
        ...
        # 实现数据的读取
        for sample_gaic, sample_cpc in zip(data_loader_train_gaic, data_loader_train_cpc):
            ...
            # tensor_list 存储多张图片的tensor, roi_cpc存储一个 roi的列表, avg_score为图片的平均分数，user_score为图片的用户打分
            cpc_tensor_list = []
            roi_cpc = []
            avg_score_list = []
            user_score_list = []
            ...
            # 使用torch.stack沿着第0维度（新的维度）堆叠这些张量
            ...
            # ----------------------  模型计算   ----------------------#
            # ---------------------------------------------------------#
            out_gaic, out_cpc, MOS_gaic, user_score_list, avg_score_list = net(
                image_gaic, roi_gaic, cpc_stacked_tensors, roi_cpc, MOS_gaic, user_score_list, avg_score_list
            )

            # forward
            # loss_gaic 的通用裁剪 loss这里采用 SL1 loss。
            loss_gaic = torch.nn.SmoothL1Loss(
                reduction='mean')(out_gaic, MOS_gaic)
            ...
            writer.add_scalar('Loss/CPC', loss_cpc.item(),
                              epoch * len(data_loader_train_cpc) + bs_id)
            ...
            sys.stdout.write('\r[Epoch %d/%d] [Batch %d/%d] [Train Loss: %.4f]' %
                             (epoch, 79, bs_id, len(data_loader_train_cpc), avg_loss))
        ...
        # torch.save(net.module.state_dict(), args.save_folder + '/' + repr(epoch) + '_%.3f_%.3f_%.3f_%.3f_%.3f_%.3f_%.3f_%.3f_%.3f_%.3f' %
        #            (acc4_5[0], acc4_5[1], acc4_5[2], acc4_5[3], acc4_10[0], acc4_10[1], acc4_10[2], acc4_10[3], avg_srcc, avg_pcc) + '.pth')
