...
from dataloader.augmentations import CropAugmentation
...
class GAICD(data.Dataset):
    ...
    def __init__(self, image_size=256, dataset_dir='dataset/GAIC/', set='train',
        ...
        for image in image_lists:
            self._imgpath.append(os.path.join(
                self.dataset_dir, 'images', set, image))
            self._annopath.append(os.path.join(
                self.dataset_dir, 'annotations', set, image[:-3]+"txt"))
    ...
    def __getitem__(self, idx):
        ...
        for annotation in annotations_txt:
            ...
            if float(annotation_split[4]) != -2:
                annotations.append([float(annotation_split[0]), float(annotation_split[1]), float(
                    annotation_split[2]), float(annotation_split[3]), float(annotation_split[4])])

        # annotations 的顺序为 ymin xmin ymax xmax
...
class TransformFunctionTest(object):
    ...
    def __call__(self, image, image_size):
        ...
        for bbox in bboxes:
            source_bboxes.append([round(bbox[0] * scale_height), round(bbox[1] * scale_width),
                                 round(bbox[2] * scale_height), round(bbox[3] * scale_width)])
                ...
                for y2 in range(8, 12):
                    if (x2-x1)*(y2-y1) > 0.4999*bins*bins and (y2-y1)*step_w/(x2-x1)/step_h > 0.5 and (y2-y1)*step_w/(x2-x1)/step_h < 2.0:
                        annotations.append([float(
                            step_h*(0.5+x1)), float(step_w*(0.5+y1)), float(step_h*(0.5+x2)), float(step_w*(0.5+y2))])
        ...
        if out_h < h and out_w < w and out_h*out_w > 0.4*h*w:
            for w_start in range(0, w-out_w, w_step):
                for h_start in range(0, h-out_h, h_step):
                    annotations.append([float(h_start), float(w_start), float(
                        h_start+out_h-1), float(w_start+out_w-1)])
    return annotations
