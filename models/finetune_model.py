...
class persionality_crop_model_multi_scale_shared_attention(nn.Module):
    ...
    def __init__(self, alignsize=8, reddim=32, loadweight=True, model=None, downsample=4, num_classes=945, gen=True, per=True):
    ...
    def forward(self, data_gaic, data_cpc):
        ...
        # 获取FC_layers_1中conv3之前的特征
        # feature_gaic_128 = self.FC_layers_1[0:3](final_feat)
        # prediction_1 = self.FC_layers_1[3](feature_gaic_128)
        ...
        if self.gen == True and self.per == True:
            return (prediction_1.squeeze() + prediction_3.squeeze()).squeeze()
...
class persionality_attention_visual(nn.Module):

    def __init__(self, alignsize=8, reddim=32, loadweight=True, model=None, downsample=4, num_classes=945, gen=True, per=True):
        super(persionality_attention_visual, self).__init__()
...
def build_crop_model(alignsize=8, reddim=32, loadweight=True, model=None, downsample=4, num_classes=945, attention=False, gen=True, per=True, visual=False):

    if attention is True:
        if visual is True:
            return persionality_attention_visual(alignsize, reddim, loadweight, model, downsample, num_classes, gen, per)
        else:
