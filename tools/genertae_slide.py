...
stride = 10
scales = [0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60, 0.55, 0.5]
                ...
                for j in range(max(5, int(hh/stride))):
                    ...
                    subcrop.append(
                        [
                            ...
                            float(x+w)/image_width, float(y+h)/image_height
                        ])
...
def bbox_nms(crops, nms_threshold):
    ...
    keep_bboxes = np.ones(len(crops)).astype(np.bool)
...
if __name__ == "__main__":
    ...
    with open('pdefined_anchor.pkl', 'wb') as f:
